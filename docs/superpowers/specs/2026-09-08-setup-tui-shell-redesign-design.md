...
- In Configure, `Tab`/Shift-Tab and `←→` change group, `1`–`6` jump to one, and `↑↓` flow across group boundaries (the field list is contiguous). The sidebar lists the groups numbered and its bar eases to the current one. The sidebar itself never takes key focus inside Configure.
